...
_is_first_note = True      # 标记本次录制是否还没有记录过音符（第一个音符 interval 记 0）

# LED闪烁时长常量（与 leds.flash 的 duration_ms 保持一致，避免硬编码分散）
LED_FLASH_MS = 100
...
def main():
    """主入口：初始化硬件后进入事件循环。"""
    global _recording, _last_note_ticks, _recorded_notes, _is_first_note
    ...
    while True:
        try:
            ...
            if ctrl_event is not None:
                print("[DEBUG] 检测到控制键事件:", ctrl_event)
            ...
            if note is not None:
                ...
                if _recording:
                    now = time.ticks_ms()
                    if _is_first_note:
                        interval = time.ticks_diff(now, _last_note_ticks) // 2#音符只取1/7间隔，避免回放开头长等待
