"""
buttons.py - 外接7按键 + 八度切换 + 录制/回放按键模块
======================================================
封装J4排针接入的7个钢琴按键（do/re/mi/fa/so/la/xi），
板载KEY1/KEY2用于八度切换，外接GPIO16/GPIO17用于录制/回放。
...
GPIO映射（均使用内部上拉 PULL_UP，按下为低电平）：
  ...
  八度键:
    ...
    KEY2 → GPIO34（八度-1）
  录制/回放键:
    GPIO16 → 录制切换（第一次按下开始录制，第二次按下结束录制）
    GPIO17 → 回放（按下播放已录制内容，录制中按下无效）
"""
...
# 录制/回放键 → GPIO引脚号
_RECORD_KEY_MAP = {
    'record': 16,     # GPIO16 → 录制切换键
}

_PLAYBACK_KEY_MAP = {
    'playback': 17,   # GPIO17 → 回放键
}
...
_record_pins = {}    # 录制键名 → Pin对象
_record_states = {}  # 录制键名 → 状态: 'idle' | 'fired'
_playback_pins = {}  # 回放键名 → Pin对象
_playback_states = {}  # 回放键名 → 状态: 'idle' | 'fired'
...
def init():
    ...
    for name, gpio in _RECORD_KEY_MAP.items():
        pin = Pin(gpio, Pin.IN, Pin.PULL_UP)
        _record_pins[name] = pin
        _record_states[name] = 'idle'

    for name, gpio in _PLAYBACK_KEY_MAP.items():
        pin = Pin(gpio, Pin.IN, Pin.PULL_UP)
        _playback_pins[name] = pin
        _playback_states[name] = 'idle'
...
def get_record_key_event():
    """
    扫描录制切换按键（GPIO16），返回本次新按下的事件类型。

    第一次按下：开始录制（清空旧数据）
    第二次按下：结束录制

    Returns:
        str | None: 'record'（GPIO16按下），无新按下时返回 None
    """
    for name, pin in _record_pins.items():
        if _check_edge(pin, _record_states, name):
            return name
    return None


def get_playback_key_event():
    """
    扫描回放按键（GPIO17），返回本次新按下的事件类型。

    按下时回放已录制内容，录制中按下无效（由上层 piano.py 判断）。

    Returns:
        str | None: 'playback'（GPIO17按下），无新按下时返回 None
    """
    for name, pin in _playback_pins.items():
        if _check_edge(pin, _playback_states, name):
            return name
    return None
